(ns cisco_webex.main
  "Kotodama WASM entrypoint for the Cisco_webex clean-room actor (L4) — Clojure port.
  ...
  facts (`cisco_webex.<Entity>/<field>`); `*store*` is the in-memory materialization
  used by the contract test and by the WASM runtime before a live engine binds."
  (:require [clojure.string :as str]))

(def ns-prefix "cisco_webex")
(def tier "L4")
...
(defn emit-facts
  "EAVT facts for one record: {\"cisco_webex.<Entity>/<field>\" v ...}. The datomic
...
(defn healthz [] [{:status "ok" :actor "cisco_webex-compat" :tier tier :entities entities} 200])
...
(defn start! [] :cisco_webex-compat/ready)
